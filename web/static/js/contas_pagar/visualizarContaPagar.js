...
export function setupVisualizarContaPagar() {
    window.abrirModalVisualizacao = function (id) {
        ...
        document.getElementById('view_valor_original').textContent = new Intl.NumberFormat('pt-BR', { style: 'currency', currency: 'BRL' }).format(debito.valor_original);
        document.getElementById('view_saldo_restante').textContent = new Intl.NumberFormat('pt-BR', { style: 'currency', currency: 'BRL' }).format(debito.saldo_restante);
        document.getElementById('view_dt_pagamento').textContent = debito.dt_pagamento ? debito.dt_pagamento.substring(0, 10).split('-').reverse().join('/') : '-';
